(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    // let sortedArr = array;
    // if(!isSorted) sortedArr = array.sort();
    ...
    //   if(!output[number]) output[number] = 0;
    //   else {
    //     output[number] ++;
    //   }
    // })
    //
    // _.each(output, function(_, key) {
    //   uniqArr.push(Number(key));
    // })
    // return uniqArr;

    if(!iterator) iterator = _.identity;
    let sortedArr = isSorted ? array : array.sort();
    let result = [];
    let temp = sortedArr[0];

    for(let i = 1; i < sortedArr.length; i ++) {
      if(iterator(sortedArr[i]) !== iterator(temp)) {
        result.push(temp);
        temp = sortedArr[i];
      }
    }
